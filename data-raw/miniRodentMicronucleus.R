...
miniRodentMicronucleus <-
    dplyr::select(TGGATES, Rodent.Micronucleus, top_features) %>%
    dplyr::rename(Genotoxic = Rodent.Micronucleus) %>%
    dplyr::mutate(Genotoxic = forcats::as_factor(Genotoxic))

usethis::use_data(miniRodentMicronucleus)
