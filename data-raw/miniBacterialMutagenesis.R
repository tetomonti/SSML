...
miniBacterialMutagenesis <-
    dplyr::select(TGGATES, Bacterial.Mutagenesis, top_features) %>%
    dplyr::rename(Genotoxic = Bacterial.Mutagenesis) %>%
    dplyr::mutate(Genotoxic = forcats::as_factor(Genotoxic))

usethis::use_data(miniBacterialMutagenesis)
